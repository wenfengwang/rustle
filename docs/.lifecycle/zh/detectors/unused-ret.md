...
在此示例中，合约没有检查 `remove` 函数的返回值，以确保 `guardian` 存在于 `self.guardians` 中。如果 `guardian` 不存在，程序不会产生 panic，这可能会带来意想不到的影响。
...
在这个版本中，`remove_guardians` 将返回所有不在 `self.guardians` 中的 `guardians`。
