...
合约在进行跨合约调用之前应始终先更改状态，并在跨合约调用失败时，在回调函数中回滚这些更改。

### 示例代码

以下是一个重入攻击的例子。受害合约仅在成功向攻击者转账后，才在函数`ft_resolve_transfer`中更新状态（即`attacker_balance`）。

受害合约调用`ft_token::ft_transfer_call`来转账，该调用在内部转账完成后会触发攻击者的`ft_on_transfer`。

然而，如果攻击者的`ft_on_transfer`再次调用受害合约的`withdraw`方法，由于状态（即`attacker_balance`）尚未更新，受害合约会再次向攻击者转账。
...
正确的实现应该是在调用外部函数之前改变状态，并且只有在Promise失败时才恢复状态。
