...
mod upgrade {
    ...
    pub extern "C" fn upgrade() {
        ...
        unsafe {
            ...
            // 在迁移完成后安排返回配置。
            //
            // 升级方法将其作为一个操作附加，因此如果配置视图调用无法成功返回，则可以回滚整个升级，包括部署合约操作和迁移。视图调用反序列化状态并反序列化包含owner_id的配置。如果合约能够反序列化当前配置，那么它可以验证所有者并再次执行升级（以防之前的升级/迁移出现问题）。
            //
            // 这是远程合约升级的额外安全保障。
            sys::promise_batch_action_function_call(
                promise_id,
