...
コントラクトにはアップグレード機能が必要な場合があります。**Rustle** はそのようなインターフェースがコントラクトに存在するかどうかをチェックします。

アップグレード機能がなければ、コントラクトをアップグレードすることはできず、コントラクトの状態を移行することもできません。
...
mod upgrade {
    ...
    /// 自己アップグレードとmigrateの呼び出しを行い、コードをメモリにロードしないことでガスを最適化します。
    ...
    pub extern "C" fn upgrade() {
        ...
        unsafe {
            ...
            // owner_idを含む設定をデシリアライズします。コントラクトが現在の設定をデシリアライズできる場合、
