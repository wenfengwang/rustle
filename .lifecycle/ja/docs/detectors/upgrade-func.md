...
コントラクトはアップグレード機能が必要な場合があります。**Rustle** はそのようなインターフェースがコントラクト内に存在するかどうかをチェックします。

アップグレード機能がなければ、コントラクトをアップグレードすることができず、コントラクトの状態を移行することもできません。

### サンプルコード

アップグレード機能のサンプルを以下に示します。
...
mod upgrade {
    ...
    /// 自己アップグレードと移行を呼び出し、コードをメモリにロードしないことでガスを最適化します。
    /// 入力としては、シリアライズされていないコードのバイト列を取ります。
    #[no_mangle]
    pub extern "C" fn upgrade() {
        ...
        unsafe {
            ...
            // 移行が完了した後に設定を返すスケジュールを設定。
            //
            // アップグレードメソッドはそれをアクションとして添付するので、設定のビュー呼び出しが
            // 正常に返されない場合、デプロイコントラクトアクションと移行を含むアップグレード全体を
            // ロールバックすることができます。ビュー呼び出しは状態をデシリアライズし、owner_idを
            // 含む設定をデシリアライズします。コントラクトが現在の設定をデシリアライズできる場合、
            // オーナーを検証し、アップグレードを再実行することができます（前回のアップグレード/移行が
            // 不調だった場合）。
            //
            // これはリモートコントラクトアップグレードのための追加の安全対策です。
            sys::promise_batch_action_function_call(
                promise_id,
