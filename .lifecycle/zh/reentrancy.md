...
受害合约调用 `ft_token::ft_transfer_call` 来转移代币，该调用在内部转账后会触发攻击者的 `ft_on_transfer`。

然而，如果攻击者的 `ft_on_transfer` 再次调用受害者的 `withdraw` 函数，由于状态（即 `attacker_balance`）尚未改变，受害者会再次向攻击者转账。
...
正确的实现是在调用外部函数之前更改状态，并且只有在 Promise 失败时才恢复状态。
