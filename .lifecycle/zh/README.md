...
在 Linux 中，使用以下命令为 **Rustle** 安装所需的工具包。这些命令在 Ubuntu 20.04 LTS 中经过测试。
...
以下命令适用于 macOS 用户，它们仅在 Apple Silicon Mac 上经过测试，因此请谨慎使用。
...
* `output`：将在其中生成审计报告的路径。默认为 `./audit-result`。
...
# [可选] 使用指定的检测器或严重性组在 `~/linear-report` 中保存审计报告上运行 Rustle
...
| `non-private-callback`  | [缺少回调函数的宏 `#[private]`](docs/detectors/non-private-callback.md) | 高     |
| `reentrancy`            | [查找易受重入攻击的函数](docs/detectors/reentrancy.md)     | 高     |
| `unsafe-math`           | [缺少算术运算的溢出检查](docs/detectors/unsafe-math.md)            | 高     |
...
| `upgrade-func`          | [合约中缺少升级功能](docs/detectors/upgrade-func.md)                           | 低      |
...
1. 将此存储库分叉到您的帐户。
2. 将新的检测器放在 [/detectors](/detectors/) 下。
...
## 注意
**Rustle** 可以在开发过程中用于迭代扫描 NEAR 智能合约。这可以节省大量的手动工作并减轻部分潜在问题。然而，复杂逻辑中的漏洞或与语义相关的问题仍然是 **Rustle** 的局限性。要定位复杂的语义问题，需要 [BlockSec](https://blocksec.com/) 的专家进行彻底和全面的审查。请通过 [联系我们](audit@blocksec.com) 获取审核服务。

## 许可证

该项目采用 AGPLv3 许可证。请参阅 [LICENSE](LICENSE) 文件以获取完整的许可文本。